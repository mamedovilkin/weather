    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        provideContent {
            ...
            LaunchedEffect(location) {
                if (temperatureUnit != null && location != null) {
                    widgetUseCase
                        .getCurrentWeather(location!!.first(), location!!.last(), temperatureUnit!!.convertToTemperatureUnit())
                        .onSuccess { weather ->
                            icon = weather.icon
                            temperature = context.getString(R.string.temperature, ceil(weather.temperature).toInt(), temperatureUnit?.convertToTemperatureUnit()?.convertToSymbol())
                            summary = context.getString(R.string.widget_summary, weather.description, ceil(weather.feelsLike).toInt(), temperatureUnit?.convertToTemperatureUnit()?.convertToSymbol())
                            isLoading = false
                        }
                        .onFailure {
                            isLoading = false
                    ...
                    Box {
                        ...
                        if (isLoading) {
                            ...
                            Column(
                                ...
                                Image(
                                    provider = ImageProvider(getWeatherIcon(icon)),
                                    contentDescription = null,
                                    contentScale = ContentScale.Crop,
